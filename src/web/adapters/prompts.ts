...
export const PROMPTS = {
  ...
  visualPlan: [
    [
      ...
      'how is "picture" with the name of the library picture that is that',
      'thing (a clinic, a leaf, a bucket, a server), or "shape" with',
  ...
  visualScript: [
    [
      'You write a short visual lesson as a structure. You never place',
      'anything: the app lays the picture out and draws it. You say what',
      'is in the picture, how the parts relate, what the voice says, and',
      'on which word each part appears.',
    ].join(' '),
    [
    ...
    [
      'Items, four to ten; a picture a student takes in at a glance has',
      'few parts. A hub holds up to four inputs and four outputs; a flow',
      'or cycle three to six steps; a compare up to four a side; layers',
      'up to five. Each item has an id, a role and a kind. Roles:',
      'centre (one, for a hub), input and output (a hub), step (flow and',
      'cycle, in order), left and right (compare, in order), layer',
      '(layers, bottom first), title (one short heading, optional), note',
      '(one short remark under the picture, optional). Kinds: picture, a',
      'drawn thing from the library, with picture set to its name from',
      'the catalogue and text as the name a student reads under it, one',
      'to three words; chip,',
      'a rounded tag with text of up to twenty-two characters, one or two',
      "words in the chapter's own words, never a word cut short; label, a",
      'plain line of text for a title or a note; dots, a scatter of',
      'count small circles inside the centre, for things that are many',
      '(tokens, cells, people). The centre of a hub is a picture when the',
      'plan names one; a box with a word in it is a step, never the',
      'centre. Use a picture wherever the library has the thing the',
      'chapter names or something close to it, on the sides too: a',
      'clinic for a health service, a server for a system, a person for',
      'a patient, a tap for water. A scene with no picture at all is a',
      'poor one; aim for two or three.',
      'Colours: green, amber, blue, violet, orange, red, ink, muted, or',
      'null to let the app choose. Every field not set is null.',
    ].join(' '),
    [
      'Arrows join two items by id, from and to, with a colour or null,',
      'double true when it goes both ways. In a hub every input points to',
      'the centre and the centre points to every output; in a flow or',
      'cycle each step points to the next and nowhere else; a compare',
      'usually has none.',
    ].join(' '),
    [
      ...
      'belong in chips, never in the narration. The narration says only',
    ...
    [
      ...
      'fade (appears), hide, dim, undim, pulse, flow (moving dashes along',
      'an arrow, until hidden), highlight (an outline), clear (target "*",',
      'hides everything). Introduce each item and arrow with draw or fade',
      'on the word where the narration first mentions it; never pulse,',
      'flow, dim or highlight something before it is shown. Every item and',
      'arrow must be shown at least once. Never more than eight things on',
      'screen at once: hide or dim what the narration has finished with.',
      'Every chip and label text must be built from words in the chapter,',
      "the chapter's own names for things, never category names of your",
      'own.',
    ].join(' '),
    [
      'Two complete lessons, the shape to aim for. The first:',
      '{"title": "How photosynthesis works", "template": "hub", "items": [{"id": "title", "role": "title", "kind": "label", "text": "Photosynthesis", "picture": null, "color": null, "count": null}, {"id": "leaf", "role": "centre", "kind": "picture", "text": "leaf", "picture": "leaf", "color": "green", "count": null}, {"id": "chloro", "role": "centre", "kind": "dots", "text": "chloroplasts", "picture": null, "color": "green", "count": 6}, {"id": "sun", "role": "input", "kind": "picture", "text": "sunlight", "picture": "sun", "color": "amber", "count": null}, {"id": "co2", "role": "input", "kind": "chip", "text": "CO2", "picture": null, "color": "violet", "count": null}, {"id": "water", "role": "input", "kind": "picture", "text": "water", "picture": "drop", "color": "blue", "count": null}, {"id": "glucose", "role": "output", "kind": "chip", "text": "glucose", "picture": null, "color": "orange", "count": null}, {"id": "o2", "role": "output", "kind": "chip", "text": "O2", "picture": null, "color": "blue", "count": null}], "arrows": [{"id": "lightArrow", "from": "sun", "to": "leaf", "color": "amber", "double": null}, {"id": "co2Arrow", "from": "co2", "to": "leaf", "color": "violet", "double": null}, {"id": "waterArrow", "from": "water", "to": "leaf", "color": "blue", "double": null}, {"id": "glucoseArrow", "from": "leaf", "to": "glucose", "color": "orange", "double": null}, {"id": "o2Arrow", "from": "leaf", "to": "o2", "color": "blue", "double": null}], "segments": [{"text": "Plants make their own food through a process called photosynthesis.", "cues": [{"at": 0, "do": "draw", "target": "leaf"}, {"at": 8, "do": "fade", "target": "title"}]}, {"text": "It happens inside the leaf, in tiny green structures called chloroplasts.", "cues": [{"at": 4, "do": "pulse", "target": "leaf"}, {"at": 7, "do": "fade", "target": "chloro"}]}, {"text": "First, the leaf captures energy from sunlight.", "cues": [{"at": 3, "do": "draw", "target": "sun"}, {"at": 5, "do": "draw", "target": "lightArrow"}, {"at": 6, "do": "flow", "target": "lightArrow"}]}, {"text": "Next, it pulls in carbon dioxide from the air through tiny pores.", "cues": [{"at": 4, "do": "fade", "target": "co2"}, {"at": 6, "do": "draw", "target": "co2Arrow"}, {"at": 8, "do": "flow", "target": "co2Arrow"}]}, {"text": "Water travels up from the roots to reach the leaf.", "cues": [{"at": 0, "do": "fade", "target": "water"}, {"at": 1, "do": "draw", "target": "waterArrow"}, {"at": 3, "do": "flow", "target": "waterArrow"}]}, {"text": "Using the energy of the sun, the leaf turns these ingredients into glucose, a sugar that fuels the plant.", "cues": [{"at": 4, "do": "pulse", "target": "sun"}, {"at": 7, "do": "pulse", "target": "leaf"}, {"at": 12, "do": "draw", "target": "glucoseArrow"}, {"at": 13, "do": "fade", "target": "glucose"}, {"at": 15, "do": "flow", "target": "glucoseArrow"}]}, {"text": "Oxygen is released as well, and that is the air we breathe.", "cues": [{"at": 0, "do": "draw", "target": "o2Arrow"}, {"at": 1, "do": "fade", "target": "o2"}, {"at": 2, "do": "flow", "target": "o2Arrow"}, {"at": 9, "do": "pulse", "target": "o2"}]}, {"text": "So in one line, carbon dioxide plus water plus light becomes glucose and oxygen.", "cues": [{"at": 0, "do": "hide", "target": "title"}, {"at": 4, "do": "pulse", "target": "co2"}, {"at": 7, "do": "pulse", "target": "water"}, {"at": 9, "do": "pulse", "target": "sun"}, {"at": 11, "do": "pulse", "target": "glucose"}, {"at": 13, "do": "pulse", "target": "o2"}]}]}',
      'The second:',
      '{"title": "The token bucket", "template": "hub", "items": [{"id": "bucket", "role": "centre", "kind": "picture", "text": "bucket", "picture": "bucket", "color": "blue", "count": null}, {"id": "tokens", "role": "centre", "kind": "dots", "text": "tokens", "picture": null, "color": "amber", "count": 5}, {"id": "refill", "role": "input", "kind": "chip", "text": "fixed rate", "picture": null, "color": "green", "count": null}, {"id": "request", "role": "input", "kind": "picture", "text": "request", "picture": "arrows", "color": "violet", "count": null}, {"id": "pass", "role": "output", "kind": "chip", "text": "passes", "picture": null, "color": "green", "count": null}, {"id": "empty", "role": "note", "kind": "label", "text": "empty: requests wait", "picture": null, "color": "red", "count": null}], "arrows": [{"id": "refillArrow", "from": "refill", "to": "bucket", "color": "green", "double": null}, {"id": "take", "from": "request", "to": "bucket", "color": "violet", "double": null}, {"id": "out", "from": "bucket", "to": "pass", "color": "green", "double": null}], "segments": [{"text": "A token bucket holds a small number of tokens.", "cues": [{"at": 2, "do": "draw", "target": "bucket"}, {"at": 6, "do": "fade", "target": "tokens"}]}, {"text": "The bucket refills at a fixed rate, one token at a time.", "cues": [{"at": 2, "do": "fade", "target": "refill"}, {"at": 4, "do": "draw", "target": "refillArrow"}, {"at": 6, "do": "flow", "target": "refillArrow"}]}, {"text": "Each request that arrives takes one token out of the bucket.", "cues": [{"at": 1, "do": "fade", "target": "request"}, {"at": 4, "do": "draw", "target": "take"}, {"at": 9, "do": "pulse", "target": "bucket"}]}, {"text": "While tokens remain, the request passes at once.", "cues": [{"at": 4, "do": "draw", "target": "out"}, {"at": 5, "do": "fade", "target": "pass"}, {"at": 6, "do": "flow", "target": "out"}]}, {"text": "When the bucket is empty, requests wait for the next token or are dropped.", "cues": [{"at": 4, "do": "dim", "target": "tokens"}, {"at": 6, "do": "fade", "target": "empty"}]}, {"text": "That one rule is how a token bucket limits the rate of requests.", "cues": [{"at": 6, "do": "highlight", "target": "bucket"}]}]}',
    ].join('\n'),
    [
      'When asked to mend a lesson, change only what the problems name and',
