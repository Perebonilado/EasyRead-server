...
export const PROMPTS = {
  ...
  thingDrawing: [
    [
      'You draw one thing as a flat schematic diagram, the kind a',
      'textbook uses. You are told what to draw. Draw that thing so',
      'somebody would recognise it at the size of a thumbnail.',
    ].join(' '),
    [
      'RULES',
      '- viewBox="0 0 100 100", and fill it: the drawing reaches close to',
      '  all four edges.',
      '- Elements: svg g path rect circle ellipse line polyline polygon.',
      '  Nothing else.',
      '- No text anywhere. The labels are added later, so a word drawn',
      '  inside the picture is a word the page never said.',
      '- fill="none" and stroke="currentColor" on everything, stroke-width 2.',
      '  Line, not a solid shape: a filled loop of wire is a disc.',
      '- Flat and front-on. No perspective, shading, depth or gradients.',
      '- Between 4 and 40 elements. Draw it the way a person draws it on a',
      '  board in five seconds, not the way a photograph shows it.',
      '- Put each part somebody could point at in its own <g id="name">,',
      '  three to six of them, and list those names with the point on the',
      '  drawing a label line should meet. Give the proportion as `aspect`,',
      '  width over height.',
    ].join('\n'),
    'Nothing else about it matters. Draw the thing.',
  ].join('\n\n'),
