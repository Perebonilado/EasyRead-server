...
export const PROMPTS = {
  ...
  /**
   * A story's page as a screenplay: its characters play the page, saying
   * their own lines to one another, and the narrator comes in only now and
   * then. Code stages it: who stands where, the camera, the acting.
   */
  sceneScreenplay: [
    [
      'You adapt one page of a story into a short screenplay for an',
      'animated film. Its characters play the page: they say their own',
      'lines to one another, in their own voices, and a narrator comes in',
      'only now and then. They are cartoon figures drawn once for the whole',
      'book, standing on a stage in front of a painted scene; code places',
      'them, moves the camera, and makes them look at whoever speaks,',
      'gesture, nod and move their mouths. Nothing on the stage is',
      'labelled or captioned: the story says who everyone is.',
    ].join(' '),
    [
      'beats are the screenplay, in the order the page tells it. Three',
      'kinds. "line": a character speaks. who is their id; to is whom they',
      'say it to, or null; say is exactly the words the book gives them,',
      'and nothing else: no quotation marks, no "she said", no words of',
      'the narrator. A line is only what the book quotes someone saying:',
      "never the narrator's words, never words the book reports rather than",
      'quotes ("he asked what was going on"), and never a question to the',
      'viewer: the stage\'s check ("Think: …", "Can you spot …?") is always',
      'narration. The narrator never says what the book quotes a character',
      'saying: that is their line. Where the book quotes no one, the',
      'characters may say a few short lines of what it says they said.',
      "Keep every line the book's characters say on this",
      'page, in order and word for word; a long speech may be split into',
      'lines of a sentence or two, never reworded or shortened. state is',
      'the face they say it with ("neutral", "happy", "sad", "angry",',
      '"afraid", "surprised", "thinking", "pain"), or null to keep theirs;',
      'pace is "calm", "quick", "slow", "whisper" or "shout", or null.',
      'from is where the line comes from: "here" when they say it on the',
      'stage; "off" from just off it (a voice calling from outside);',
      '"above" from heaven or the sky; "phone" down a phone or a radio, with',
      'to the one on the stage who hears it; "letter" a letter or a note',
      'read out, who its writer and to whoever holds it; "thought" thought',
      'and not said aloud; "dream" in a dream or a memory. place on a line',
      "is null, except where the page's people are in two places at once and",
      'speak between them (Sally trapped in a cave, the boys above it): then',
      "each line's place is where its speaker is, and the stage cuts between",
      'the two places. A line the book',
      'gives to "a voice from heaven" is from "above", said by the voice',
      "from above in the story's characters, never by anyone on the stage.",
    ].join(' '),
    [
      '"action": something the stage shows without words, as it happens',
      "between the lines. who does it: a character's id, or a thing's id",
      'for a thing that changes. do is one of: "enter" (they walk on),',
      '"leave" (they walk off), "hug", "reach" (a touch, a poke, a hand held',
      'out, something given), "point", "look", "wave", "nod", "shake" (a',
      'shake of the head), "laugh", "hop" (a jump for joy), "clap", "sob",',
      '"shrug", "lean-in", or "still" (only a face or a thing changes). to',
      'is whom or what it is toward, or null ("look" or "point" up at the',
      'sky: to null and say "up at the sky"). state is a face, or a sign',
      'they now show ("shivering", "shaking", "dizzy", "coughing",',
      '"sleeping", "breathless", "tears", "sweating", "fever", "headache",',
      '"idea"), or the new state of a thing (the flame "out"). show is a',
      'thing that comes into view here, by id. hold is how long it takes in',
      'seconds, 0.6 to 3: quiet in which music plays. say is a few words',
      'saying what happens, for the record: never spoken.',
    ].join(' '),
    [
      '"narration": the narrator, briefly, and only for what the stage',
      'cannot show: where and when, when that changes; time passing; a',
      'sound or a happening off the stage; what someone feels or thinks',
      'inside. One short sentence, sixteen words at most, in the present',
      'tense. Never repeat a line, never say who says what, and never tell',
      'what an action shows: "Kofi groans" is an action, not narration.',
      'A page may hold several scenes: open each with a narration that says',
      'where and when, place its place (the same one for a later time), and',
      'with who is there in it by id; with is null on every other beat. Each',
      "scene shows only its own people. place is a place's id when the scene",
      'moves there, else null; who and',
      'state may give the face of whoever the narration is about ("Mira\'s',
      'heart jumps": who "mira", state "afraid").',
    ].join(' '),
    [
      'The characters carry the page. On a page that is mostly talk, the',
      'narrator says a third of the words or fewer: often a sentence to set',
      'the scene, and a few words where time passes. A page of little talk',
      'is carried by actions and the few lines there are, with a short',
      'narration where the stage cannot show what happens. Where the book',
      'tells something the stage can show (someone laughs, groans, pokes',
      'their brother, looks up at the sky), make it an action.',
    ].join(' '),
    [
      'opening lists who and what is in the scene as the page opens, by id:',
      'the characters there in its first scene only, and a thing already in',
      'view; never someone the page brings in later. Someone who',
      'arrives later comes on with an "enter" action, and someone who goes',
      'leaves with "leave". Leave out of opening anyone the page has arrive',
      '("footsteps hurry along the quay: it is Tobi"): they come on with',
      '"enter" where the page says so. Two who go together ("together they',
      'walk home") leave in one action, who one of them and to the other.',
      'Everyone in the scene stays on the stage until',
      'the words take them off it or the next scene begins: never take someone off to show someone',
      'else alone. The camera makes the close shots on its own. A character',
      'who is only heard, a voice from above, or a group is never in',
      'opening and never enters or leaves: their lines come from where they',
      'are ("off", "above", "phone", "letter", "dream").',
    ].join(' '),
    [
      "cast lists every one and every thing the screenplay needs. A story's",
      'character is kind "character" with ref set to their id: never say how',
      'they look, since each is drawn once for the whole book; state is the',
      'face they come on with, or null to keep the one the last page left',
      'them with; pose ("standing", "hand on head", "hands on belly", "hand',
      'on mouth", "arms up", "pointing", "waving", "holding", "lying", "in',
      'bed"), signs and holding (one of "book", "phone", "cup", "umbrella",',
      '"bag", "ball", "lantern", "flag", "magnifier") only when the page has',
      'them so. Someone the story does not name (a shopkeeper, a',
      'fisherman) is kind "person" with a figure. A thing the story needs',
      'to show (the conch, the box of matches) is kind "drawing": brief',
      'says what to draw, with no people and no words in it; states lists',
      'its looks that change ("lit": a small bright flame), at most three;',
      'parts are never labelled. A place is kind "place" with ref set to one',
      "of the story's places: the scene behind the stage.",
    ].join(' '),
    [
      'music is what plays under the voice from that beat on; null carries',
      'on as it was. Set it on the first beat, then change it where the',
      'feeling of the story turns, once or twice a page: "calm", "curious"',
      'for a discovery, "bright" for good news or success, "playful" for',
      'fun, "motion" while someone travels or something flows, "solemn" for',
      'loss or grief, "tense" for danger or suspense, "none" for a stretch',
      'that needs quiet. energy is "high" for a chase, a rush or danger',
      'close, and only with motion, bright or tense; else null.',
    ].join(' '),
    [
      'fit is "poor" only for a page with no story to play (a contents',
      "page, a copyright notice, a title page, the author's or publisher's",
      'note, a summary of the book), with fitReason saying why; else "good".',
      'title names the scene in a few words. mood is the feeling of the',
      'page: "calm", "bright", "curious", "serious" or "playful". Every',
      'field of a beat or a thing is present, null where its kind does not',
      'use it.',
    ].join(' '),
    [
      'An example of the shape of an answer, the start of a page where Ada',
      'asks her grandmother for a story. It shows the form, not words to',
      'reuse:',
      JSON.stringify({
        fit: 'good',
        fitReason: null,
        ...
        beats: [
          {
            ...
            to: null,
            from: null,
            say: 'A warm night. Ada, Kofi and their grandmother sit by the fire.',
            do: null,
            state: null,
            show: null,
            pace: null,
          ...
          },
          {
            ...
            to: null,
            from: null,
            say: 'Kofi groans and shakes his head.',
            do: 'shake',
            state: 'sad',
            show: null,
            pace: null,
          ...
          },
          {
            ...
            to: 'nana',
            from: 'here',
            say: 'Not the one about the tortoise again!',
            do: null,
            state: null,
            show: null,
            pace: 'quick',
  ...
  sceneStory: [
    ...
    'it.',
    "book is the book's own title and author, as its title page or cover",
    'gives them ("Hide-and-Seek", "T. Albert"), each null where this stretch',
    'does not show it; null for a stretch with neither.',
    'characters: every named person, or creature that acts like one, who',
    'does or says something here; and everyone else who speaks, seen or',
    'not: God, a voice from heaven, an angel heard but not seen, a voice',
    'on the phone or the radio, the writer of a letter read out, and a',
    'group that speaks or acts as one (the crowd, the disciples, the',
    'soldiers); and anyone the story does not name who speaks or acts on a',
    'page (a man with leprosy, a ruler whose daughter died, a police',
    "captain, Sally's mother), called what the story calls them. name is",
    'what the story mostly calls them; aliases the other names it uses for',
    "the same one, never another's: Mary is not an alias of Jesus, nor",
    '"Mark\'s father" of Mark; each is a character of their own. Call a character met earlier',
    ...
    'for an animal, "creature" for anything else that acts like a',
    'person (a monster, a robot, a talking teapot), and "group" for people',
    'who speak or act as one, drawn as a crowd. presence is "seen" for',
    ...
    '"creature" for an animal or a monster that talks, "divine" for God or',
    'a god, "crowd" for a group; null for one who never speaks, and for',
    "one whose words the text's tradition would not give a voice to: the",
    'narrator says them.',
    'places: where the story happens here, and a place within one where',
    'part of it plays apart from the rest (a cave under a fallen tree',
    'while the others are above it; the bottom of a well; a prison cell),',
    'each with a look an',
    'illustrator can paint as a recognisable scene with no one in it: its',
    'defining features first ("a wooden fishing boat on the Sea of',
    'Galilee, nets over the side, low brown hills across the water"), then',
    'the light and the colours. kind is "outdoor", "indoor", or "vessel"',
    'for something people ride in (a boat, a cart, a bus). stand is "in"',
    'where people are in it behind its side (in a boat, behind a table, at',
    'a well) and "on" where they stand on open ground. front is what stands',
    'in front of their legs there ("the side of the boat", "a long wooden',
    'table", "the low wall of the well"), or null. sound is what the place',
    'sounds like while the story is there ("water" for a harbour or a',
    'lake, "wind", "rain", "fire" for a hearth, "clock", "machine",',
    '"bubbles", "electric", "heartbeat"), or null for a quiet place.',
    ...
    'on every page it fits, so the places stay few; a page that gives no',
    'clue happens where the page before did, unless the story says they',
    'went somewhere. time is "dawn", "day", "dusk" or "night" where the',
    'text says or shows it ("they lay down to sleep": night), else the',
    'time of the page before unless the story says time has passed, and',
    'null only on a page with none before it.',
    'weather is "clear", "rain", "storm", "wind", "snow" or "fog" where the',
    'text says or shows it ("the wind rose": storm), else null. crowd is',
    '"many" where crowds or a busy place are about ("a great multitude",',
    '"the market was full"), "few" for a handful of others, and "none"',
    'otherwise.',
  ].join(' '),
