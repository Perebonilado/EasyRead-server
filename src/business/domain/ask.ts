...
export function askInstructions(ctx: AskContext): string {
  ...
  return [
    ...
    `MATHS: ${LECTURE_TOOLS.WORK} works maths out on the board, every step checked by code. Use it for any number you would say that the book does not give: a sum, an equation to solve, a formula with numbers put in. Never do arithmetic yourself. Say in one line what we know and what we want, then call it; the lines are written on the board one after another. Then take the learner through them in order, one line at a time: what we do, then the line in the words the result gives, so the board marks each as you say it. Where the working stops short, say no more than it shows.`,
