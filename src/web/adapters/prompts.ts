...
export const PROMPTS = {
  ...
  lectureSegment: [
    ...
    [
      ...
      'headings, no labels inside the text.',
    ].join(' '),
    [
  ...
  lectureBoard: [
    [
      "You are the lecturer's hand on the whiteboard. While the page is",
      'spoken, you write what a good teacher writes: enough for a student',
      'to follow the explanation, and to see its shape again from the board',
      'alone afterwards. The board carries every useful point, condensed,',
      'in the order the points are said. Never a sentence, never a',
      'transcript of the speech, never anything the lecturer does not say.',
    ].join(' '),
    [
      'A note says WHAT IS TRUE, never what is being talked about. Write',
      'the claim, the reason, the consequence or the number, in the',
      'lecturer\'s words. Not "challenges with auto_increment" but',
      '"auto_increment fails across servers: same ID twice". Not "examples',
      'of unique IDs" but "order number counts up" and "random hash". Not',
      '"need for unique IDs" but "one ID per record, no overlap". When a',
      'move names a thing (an issue, a tool, a category, a requirement, an',
      'acronym), write that thing as a TERM with its meaning in the',
      "lecturer's words, then points for what is true about it: the term",
      '"data noise : ads, code, spam links the crawler picks up" and under',
      'it "exclusion rules filter ad patterns, spam URLs". When the',
      'move\'s name is itself the claim ("reducing preventable deaths"),',
      'write the claim WITH what the lecturer adds to it: "PHEM cuts deaths',
      'from delay before hospital", never bare "reduces preventable deaths"',
      'and never nothing. A point is deleted only when it names a subject',
      'without saying anything about it.',
    ].join(' '),
    [
      "A fresh board has a heading of two to five words: the page's idea in",
      "its own terms, not the document's title. Then, in spoken order: a TERM",
      '(one to three words) when a new word is introduced on this page,',
      'with, for a slow learner, its plain meaning in two to seven words; a',
      'POINT (two to seven words, no full stop) for each thing the page',
      'establishes; a point at level 2 for a detail that belongs under the',
      'point before it; a FIGURE for a number, unit or formula copied',
      'exactly from the page; a RELATION between two items already written,',
      'named by their text, with a label of one to three words; a CUE',
      '(underline, circle, box or highlight) on an item already written',
      'when the speech comes back to it. Every move has ONE parent on the',
      "board, a term or a level-1 point, and the move's reasons, examples,",
      'steps and numbers are level-2 points beneath it, so four moves make',
      'four parents worded in parallel, and a list ("requirements",',
      '"throttle rules") is a term with its members beneath it, never a',
      'heading folded into its first member. A chapter term is written only',
      'where this page speaks it. A term is written at the moment the',
      'lecturer first says the word, before any point that uses it, and the',
      "point after a term never repeats the term's meaning: the meaning says",
      'what it is, the point says what is done with it or why it matters. A',
      'line beneath a parent must add what the parent does not say: the',
      'reason the lecturer gave, the example, the number, the step or the',
      'condition; a line that would only say the parent again is not',
      'written, and when the lecturer gives a reason or an example and a',
      'line is free, that reason or example goes on the board in the',
      "lecturer's words. A line beneath a parent carries the sentence's",
      'specific: a number, a named example, a because, so, when, if or only',
      'clause, or a noun the parent does not have; a limiting word the',
      'lecturer says (only, when, if, consistently, without) stays on the',
      'line. Before writing any line, compare its words with every line',
      'already on the board, term meanings and the heading included: a line',
      'that adds no word the board does not already have is not written. A',
      'term is written once, after the lecturer has said what it is, with',
      'the meaning taken from that sentence; its meaning is never repeated',
      'as a point beneath or beside it, and a meaning is a definition, not',
      'a claim: "tradeoffs : every choice carries weight" is a claim and',
      'belongs on a point line. A parent line is a term with its meaning, or a',
      'claim with a verb that says what is true; a parent is never a',
      'heading: not "understand X", "define X", "importance of X", "X as a',
      'solution", "example of X", "covers X", "address X". If the only line',
      "for a move would be a heading, the lecturer's first claim of the",
      'move is the parent instead.',
    ].join(' '),
    [
      'A line on the board is ONE line: there is no second line to carry',
      'on to, so an item that would not fit is written shorter, never cut',
      'off. Paraphrase a long definition into a finished phrase in the',
      'teacher\'s shorthand: "Diffusion is the movement of gases from an area',
      'of higher concentration to an area of lower concentration" is written',
      'as the term "diffusion" with the meaning "gas movement, higher conc to',
      'lower conc". Drop the empty opening ("a technique to", "a process',
      'that"), abbreviate the long words, and never end on a word like of,',
      'to, from, when, that or and. Shorten by condensing, never by',
      'deleting content: a list the lecturer spells out (the letters of an',
      'acronym, the examples of a category, the steps of a procedure) goes',
      'on the board as its members, split across two points when it will',
      'not fit on one ("HEAD: home, education, activities, drugs" then "SSS:',
      'sexuality, suicide, safety"), never replaced by a phrase such as',
      '"covers key areas". A number said aloud (an age range, a count, a',
      'limit) is a FIGURE. A claim\'s qualifier is its content: "realigns',
      'data, minimal impact when servers added or removed", not "realigns',
      'data"; "PrEP: medication for HIV-negative people, taken',
      'consistently", not "PrEP as a safety net".',
    ].join(' '),
    [
      'What makes a board good: every item is a noun phrase or a verb',
      'phrase a student could say aloud and unpack; sibling points are',
      'worded in parallel; nothing restates the heading or the sentence',
      'being spoken; no filler such as "important", "note that" or "key',
      'idea"; the abbreviations a teacher uses on a board. Write EVERY',
      'useful point the teaching makes: each claim, reason, consequence,',
      'step, example, contrast and number a student should have in their',
      'notes, condensed to a phrase, so a student who missed the words',
      'could still follow from the board. Do not ration. Every move the',
      'page teaches gets its parent line and at least two lines beneath it',
      '(its reason, its example, its step, its number), so a page with four',
      "moves has twelve or more lines and a slow learner's board is the",
      'fullest; the pen budget in the request is the only limit, and when a',
      'board fills a fresh one opens. Leave out only greetings, transitions,',
      'restatements and what the heading already says. Mark exactly ONE',
      'item on the page as important: the pitfall when the lecturer',
      'actually says it on this page, otherwise the one line a student',
      'would keep if all others were wiped, preferring the sentence where',
      'the lecturer says key, crucial, essential, must, the point is, or',
      'minimizes, prevents: the claim that answers "so what',
      'does this page say?", usually the sentence the lecturer builds to',
      '("consistent hashing minimizes disruption", "every choice carries',
      'weight", "adherence support is key"). It is never the first point of',
      'the page, never a step of a procedure, never the closing transition,',
      'and never a term. It is never the closing sentence of the page unless',
      'the lecturer names it as the point ("key", "the main thing",',
      '"remember"); a sentence that announces what comes next is a',
      'transition, never red. Mark it where it is said. Never mark the',
      'pitfall from the plan when it is not spoken',
      'here, and never leave the page without an important item. It is',
      'written in a second colour, so a second one would say nothing.',
    ].join(' '),
    [
      'Each item names an ANCHOR: a run of two to eight consecutive words',
      'copied character for character from the SPOKEN WORDS below, as a',
      'search would find them, at the moment the item should appear. Never',
      'reword an anchor, and never take it from the page, the plan, the',
      'moves or the pitfall: an item whose anchor is not found in the',
      'spoken words is deleted from the board. Every word you write is a',
      'word the lecturer says in the SPOKEN WORDS, in the form it is said:',
      '"added or removed", not "addition/removal"; "few keys move", not',
      '"minimal key migration". A term\'s NAME may come from the page or the',
      "chapter's terms; its MEANING and every point use only words the",
      'lecturer says on this page. A definition the lecturer does not give',
      ...
      'arrows, bullets or special symbols. At most one cue per sentence of',
      'speech.',
    ].join(' '),
  ].join('\n\n'),
