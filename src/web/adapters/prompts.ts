...
export const PROMPTS = {
  ...
  visualDirector: [
    ...
    [
      ...
      'wins. Then the card, with every field it needs, naming things by',
      "the menu's words so they draw. Then shouldSee: one sentence a",
      'judge can check against the picture. Then confidence: high, or low',
      'when you are unsure the picture will read.',
    ].join(' '),
    [
      'Lean toward what moves. Every moment moves in at least one way',
      'unless it is a title or a statement: a living thing as a figure',
      'from the menu, alive on screen; a process over time as a mechanism',
      'running its stages on the sentences; things arriving as reveals on',
      'their words; a picture with a motion (travel, bounce, hover, spin,',
      'shake); callouts that follow a part; a number that counts; a chart',
      'whose bars grow. A still card is the exception, and your reasoning',
      'says why nothing in the moment moves. A thing with parts is a',
  ...
  visualJudge: [
    ...
    'claims to draw and the brief: what a learner should see. Say,',
    'drawing by drawing, whether it looks like the thing it is named',
    'for: a fly must read as a fly, a bucket as a bucket, a cow as a',
    'cow; a plain shape or an icon that means something else does not.',
    ...
    'ask for motion the frames cannot show. Parts arrive across the',
    ...
    'fine. Answer for every moment listed, by its number.',
  ].join(' '),
