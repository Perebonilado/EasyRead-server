...
/**
 * What a thing looks like, said without ever saying what it is for. The
 * drawer is given this and never the name, which is the whole point: a
 * name says what a thing does, and asking for a kidney by name is how
 * you get a beaker.
 */
 ...
 */
export const stageNarrationSchema = z.object({
  title: z.string().min(1).max(60),
  fit: z.enum(['good', 'poor']),
  fitReason: z.string().max(160).nullable(),
  sentences: z.array(z.string().min(1).max(300)).min(8).max(48),
      ...
      z.object({
        ...
        cast: z
          .array(
            z.object({
              name: z.string().min(1).max(24),
              looksLike: z.string().max(120).nullable(),
            }),
          )
          .min(1)
          .max(6),
        beats: z
          .array(
            z.object({
              sentence: z.number().int().min(0),
              about: z.array(z.string().min(1).max(24)).min(1).max(2),
              does: z.enum([
                'introduces',
                'adds',
                'connects',
