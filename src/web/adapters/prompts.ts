...
export const PROMPTS = {
  ...
  visualScript: [
    ...
    [
      ...
      'minutes spoken, at least two hundred and fifty words and up to',
      'seven hundred; a thin narration is the most common fault. Spoken',
      ...
      'Say only what the chapter says.',
    ].join(' '),
    [
      'Then the moments. Cut the narration into moments in order, without',
      'a gap or an overlap: each covers one to four sentences, usually',
      'two or three, from and to being zero-based sentence indexes, the',
      'first starting at zero, the last ending at the last sentence. A',
      'title or a statement may take one sentence; a card that has parts',
      'to walk through takes the sentences that walk through them. Each',
      'moment is one card. title:',
      ...
      'an outline, its parts, and how it moves. Outlines: blob (a soft',
      'shape that breathes, for a cell or an island), body (a smooth',
      'swimmer), branch (a trunk dividing again and again, for a river',
      'system or a nerve), layers (bands stacked up, for rock strata or',
      'the atmosphere), lattice (nodes in a mesh, for a crystal or a',
      'molecule), vessel (a container filled to a level), terrain (ground',
      'from the side, for a valley or a coast), field (lines of flow, for',
      'wind or a current), insect (head, thorax and abdomen on legs, with',
      'wings when it has them), fish, bird, quadruped (any animal on four',
      'legs), segmented (a snake, a worm), person, plant, tree. Parts:',
      ...
      'knows the common shapes. A picture card may also carry up to three',
    ...
    [
      'A short example of the shape, for a chapter on photosynthesis:',
    ...
    [
      'When asked to mend a tutorial, change only what the problems name',
      'and keep everything else exactly as it was.',
    ].join(' '),
  ].join('\n\n'),
  lectureSketch: [
    [
  ...
  visualJudge: [
    'You look at a sheet of numbered stills from a short visual lesson,',
    'one still per moment, as a learner will see them. For each moment',
    'you are told what the card claims to draw. Say, drawing by drawing,',
    'whether it looks like the thing it is named for: a fly must read as',
    'a fly, a bucket as a bucket, a cow as a cow; a plain shape or an',
    'icon that means something else does not. Then say whether any text',
    'in the still is cut off, overlapping other text or a drawing, or',
    'too small to read, in one short sentence, else null; and whether',
    'the still is crowded, meaning a learner could not take it in at a',
    'glance. Judge only what is asked: not style, not colour, not',
    'spacing that reads fine. Answer for every moment listed, by its',
    'number.',
  ].join(' '),
