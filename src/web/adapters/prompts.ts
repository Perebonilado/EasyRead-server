...
export const PROMPTS = {
  ...
  sceneWrite: [
    ...
    [
      ...
      'spoken words; a short page gets fewer.',
    ].join(' '),
    NO_INVENTION,
    [
      'beats are the spoken sentences, one each. pause is "long" where the',
      ...
      'the feeling of the page, for the music under the voice: "calm",',
      '"bright", "curious", "serious" (illness, war, loss, anything grave)',
      'or "playful".',
    ].join(' '),
    [
      'cast lists every thing the stage shows. Three kinds on every page,',
      'and three more on the pages of a book that may use them (below):',
    ...
    [
      ...
      "and moves out at the next change of stage. say: a story's character",
      'speaks (below).',
    ].join(' '),
    [
    ...
    [
      ...
      'kind "character" with ref set to their id: never as a drawing, and',
      'never say how they look, since each is drawn once for the whole book',
      'and looks the same on every page. Their name is their caption. state',
      'is the face they come on with, "neutral", "happy", "sad", "angry",',
      '"afraid", "surprised" or "thinking", or null to keep the one the',
      'last page left them with. Change a face as the story turns, with a',
      'show effect on "id.<face>" on the words that turn it ("Ralph grins":',
      'target "ralph.happy"); the face before it goes. Point at "id.head" or',
      ...
      'stage keeps each on their own side. The things of the story (the',
      'conch, the fire) are drawings, and a place is a wide drawing with no',
      'people in it. Tell the story in order, in the present tense, as it',
      'happens on the page, and let the voice say what a character says,',
      'their words in double quotes in the sentence: "You\'re holding the',
      'matches upside down," says the fox. An effect "say" on the',
      'character, on that sentence, shows the quoted words in a bubble by',
      'their head while the voice says them. A bubble a line or two long',
      'reads best: quote the words that matter, not a whole speech.',
      'Fields: ref and state for character; null on every other kind.',
    ].join(' '),
    [
  ...
  sceneStory: [
    ...
    'time and place of the story. traits are two or three words each on',
    ...
    'illustrator can draw as an empty scene: the setting, the light, the',
    'colours.',
